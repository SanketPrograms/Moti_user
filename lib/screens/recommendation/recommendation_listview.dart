...
class _RecommendationListviewState extends State<RecommendationListview> {
  ...
  @override
  void didUpdateWidget(covariant RecommendationListview oldWidget) {
    // TODO: implement
    setState(() {
      getData();
  ...
  getData() async {
    ...
    debugPrint("this is Api $Api?userid=$userID");
        ...
        if (data["message"].toString() == "Products Not Found") {
          ...
          for (int i = 0; i < data["result"].length; i++) {
            ...
            for (int j = 0; j < data["result"][i]["opts"].length; j++) {
              ...
              if(data["result"][i]["opts"].length <= 0){
            if(data["result"][i]["opts"][0].toString().contains("variant_str")||data["result"][i]["opts"].length >= 0) {
