...
class SimilarProduct extends StatefulWidget {
  final product_id;
  SimilarProduct({Key? key, this.product_id}) : super(key: key);
...
shrinkWrap: true,
  ...
  getData() async {
    ...
    var response = await http.get(Uri.parse(("$productDetailsApi?product=${widget.product_id}&userid=$userID")));
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        for (int i = 0; i < data["result"]["related"].length; i++) {
          ProductName.add(data["result"]["related"][i]["name"]);

          productId.add(data["result"]["related"][i]["id"]);
          // productDescription.add(data["result"][i]["description"]);
          AddedToCart.add(data["result"]["related"][i]["in_cart"]);

          if (data["result"]["related"][i]["imgs"].length <= 0) {
            ProductImage.add(
                "https://upload.wikimedia.org/wikipedia/commons/0/0a/No-image-available.png");
          }
          else {
        //    for (int j = 0; j < data["result"]["related"][i]["imgs"].length; j++) {
              ProductImage.add(data["result"]["related"][i]["imgs"][0]["imgpath"]);

         //   }
          }
          for (int j = 0; j < data["result"]["related"][i]["opts"].length; j++) {
                   price.add(data["result"]["related"][i]["opts"][j]["price"].toString());
             discountprice.add(data["result"]["related"][i]["opts"][j]["dprice"].toString());
            ProductVariant.add(data["result"]["related"][i]["opts"][j]["variants"].toString());
            strVariants.add(data["result"]["related"][i]["opts"][j]["variant_str"].toString().split(":").last);
